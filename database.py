...
sql.execute('CREATE TABLE IF NOT EXISTS products (pr_id INTEGER PRIMARY KEY AUTOINCREMENT, pr_des TEXT, pr_name TEXT, pr_price REAL, pr_count INTEGER, pr_photo TEXT);')
...
## Методы для адмеестирования продуктов ##
# Добовление продукта в БД #
def pr_to_db(pr_name, pr_des, pr_price, pr_count, pr_photo):
    if (pr_name,) in sql.execute('SELECT pr_name FROM products;').fetchall():
        return False
    else:
        sql.execute('INSERT INTO products (pr_name, pr_des, pr_price, pr_count, ptphoto) VALUES (?, ?, ?, ?, ?)', (pr_name, pr_des, pr_price, pr_count, pr_photo))
        connection.commit()

# Изменения аттрибута продукта
def change_pr_attr(keyword, new_value, attr=''):
    if attr == 'name':
        sql.execute('UPATE products SET pr_name=? WHERE pr_name=?', (new_value, keyword))
...
def make_order(tg_id):
    # Достаем название товаров и их количество с корзины пользователя
    product_names = sql.execute('SELECT user_product FROM cart WHERE user_id=?;', (tg_id,)).fetchall()
    product_quantities = sql.execute('SELECT product_amount FROM car WHERE user_id=?;', (tg_id,)).fetchall()

    # Работа со складом
    product_counts = [sql.execute('SELECT pr_count FROM products pr_name=?;', (i[0,])).fetchone()[0] for i in product_names]
    totals = []
